...
- **LLM:** The LLM we use to generate content for this project is Google's Gemini. It was chosen mainly because it is the one that we explored in more detail in class, but also because it is powerful and has generous usage limits.

- **Text-to-Speech (for the Podcast):** We used ElevenLabs to perform the text-to-speech conversion of deNoise's Podcast function. Again, it was a platform that we had already used before and knew it was capable of doing what we intended.
